...
import { Brain, Lock, Loader2, Trophy } from "lucide-react";
...
export function QuizSection({ lessonId, isAuthenticated, lessonCompleted = false }: QuizSectionProps) {
  ...
  const handleSelect = (questionId: number, option: string) => {
    if (result || !lessonCompleted) return;
    setAnswers((prev) => ({ ...prev, [questionId]: option }));
  };

  const handleSubmit = async () => {
    if (!quiz || !lessonCompleted) return;
    setSubmitting(true);
    setError("");
    try {
  ...
  return (
    ...
    >
      ...
      {/* Locked state — lesson not completed */}
      {!lessonCompleted && !result && (
        <div className="p-8 text-center">
          <div className="w-14 h-14 rounded-full bg-gray-100 flex items-center justify-center mx-auto mb-3">
            <Lock size={24} className="text-gray-400" />
          </div>
          <h4 className="font-semibold text-gray-700 mb-1">Quiz Locked</h4>
          <p className="text-sm text-gray-500 max-w-sm mx-auto">
            Complete the lesson video first to unlock this quiz. Click the <strong>&quot;Complete&quot;</strong> button above after watching.
          </p>
        </div>
      )}

      {/* Unlocked — show questions */}
      {(lessonCompleted || result) && (
